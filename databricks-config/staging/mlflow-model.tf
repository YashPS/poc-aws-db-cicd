...
* [Recurring model training job](https://dbc-38930770-d64f.cloud.databricks.com#job/${databricks_job.model_training_job.id}): trains fresh model versions using the latest ML code.
...
* [Recurring batch inference job](https://dbc-38930770-d64f.cloud.databricks.com#job/${databricks_job.batch_inference_job.id}): applies the latest ${local.env} model version for batch inference.
EOF
}
